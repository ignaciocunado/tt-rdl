...
os.environ["XDG_CACHE_HOME"] = (
    "/tudelft.net/staff-umbrella/CSE3000GLTD/ignacio/relbench-ignacio/data"
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, help="The model to use (local vs global)", required=True
    )
    ...
    parser.add_argument(
        "--save_artifacts", action="store_true", help="Whether to save artifacts"
    )
    parser.add_argument(
        "--num_workers",
        type=int,
        default=8,
        help="How many workers to use for data loading. Default: 8.",
    )
    parser.add_argument(
        "--eval_freq", type=int, default=2, help="Evaluate every x epochs"
    )
    parser.add_argument("--lr", type=float, default=0.005, help="Learning rate")
    parser.add_argument("--epochs", type=int, default=10, help="Number of epochs")
    parser.add_argument(
        "--optimiser", type=str, default="adam", help="Optimizer to use"
    )
    ...
    parser.add_argument(
        "--temporal_strategy", type=str, default="uniform", help="Temporal strategy"
    )
    parser.add_argument(
        ...
        config={
            "model": (
                ...
                if args.model == "graphormer"
                    ...
                    "FraudGT"
                    if args.model == "local"
                    else "GlobalHGT" if args.model == "globalhgt" else "Arbitrary Model"
                )
            ),
        }
        | config.__dict__,
