...
@torch.no_grad()
def eval_epoch(
    loader: NeighborLoader, model: Module, task: Any, config: CustomConfig
) -> np.ndarray:
...
def train(
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        train_loss = train_epoch(
            loaders["train"], model, optimizer, loss_fn, task, config
        )
        ...
        if epoch % config.evaluation_freq == 0 or epoch == 1 or epoch == config.epochs:
            ...
            # Logging
            logging.info(
                f"Epoch: {epoch:02d}, Train loss: {train_loss:.4f}, Val metrics: {val_metrics}, Test metrics: {test_metrics}"
            )
            wandb.log(
                {
                    "epoch": epoch,
            ...
            improved = (
                config.higher_is_better and current_metric > best_val_metric
            ) or (not config.higher_is_better and current_metric < best_val_metric)
            ...
            if improved:
                ...
                if config.save_artifacts:
                    ...
                    checkpoint_filename = (
                        f"epoch_{epoch:03d}_{config.tune_metric}_{metric_value}.pt"
                    )
                    checkpoint_path = os.path.join(
                        config.checkpoint_dir, checkpoint_filename
                    )
